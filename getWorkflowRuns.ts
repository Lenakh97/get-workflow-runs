...
export const getWorkflowRuns = async (): Promise<{
	workflow_runs: ReturnType<typeof getLatestSagaStatus>[]
}> => {
	const organization = (
	...
	do {
		const reposResponse = await getInfoFromApi<
			{ name: string; default_branch: string }[]
		>(
			`https://api.github.com/organizations/${organization.id}/team/${teamID}/repos?page=${pageNumber}`,
		)
	...
	for (const repo of repos) {
		const run = await getInfoFromApi<{ workflow_runs: WorkflowRun[] }>(
			`https://api.github.com/repos/NordicSemiconductor/${repo.name}/actions/runs`,
		)
		run_promises.push(
			getLatestSagaStatus({
	...
	return {
		workflow_runs: repoRuns.flat().filter((r) => r !== undefined),
	}
}
